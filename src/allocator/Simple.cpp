...
Pointer Simple::alloc(size_t N) {
    try {
        ...
        if (*(size_t *)temp < (N + sizeof(Block) + sizeof(size_t))) { // Убираем блок выделенной памяти из списка.
            if (tmp) {
                tmp->next = temp->next;
            } else {
        ...
        temp_2 = (void **)_first_empty_descriptor;
        _first_empty_descriptor = *((void **)_first_empty_descriptor);
        *temp_2 = (void *)((size_t *)temp + 1);
        ++_number_descriptor;
        return result;
    } catch (AllocError &e) {
        std::cerr << "Allocation error type:" << e.what() << std::endl;
        throw AllocError(AllocErrorType::NoMemory, "No Memory");
    }
}
...
void Simple::realloc(Pointer &p, size_t N) {
    if (p.get()) {
        ...
        if (n >= N) {
            ...
            if ((Block *)((char *)new_block + new_block->size) == new_block->next) {
                new_block->next = first->next;
                new_block->size += first->size;
            }
        } else {
            Block *first = _first_empty_block, *second = nullptr;
            while (first && (first < p.get())) {
...
void Simple::free(Pointer &p) {
    ...
    try {
        ...
        if (!flag)
            throw AllocError(AllocErrorType::InvalidFree, "InvalidPointer");
        if (p.get()) {
            ...
            *(void **)p.get() = first;
            if (((char *)p.get() + *((size_t *)p.get() - 1) - sizeof(size_t)) == *(void **)p.get()) {
                *(Block **)p.get() = first->next;
                *((size_t *)p.get() - 1) += first->size;
            }
...
void Simple::defrag() {
    ...
    while ((void *)((char *)_first_empty_block + _first_empty_block->size) < _descriptor) {
        ...
        if ((Block *)((char *)_first_empty_block + _first_empty_block->size) == _first_empty_block->next) {
            _first_empty_block->size += _first_empty_block->next->size;
            _first_empty_block->next = _first_empty_block->next->next;
        }
    }
}
